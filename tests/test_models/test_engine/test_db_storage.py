...
class TestDBStorage(unittest.TestCase):
    ...
    def test_all_no_class(self):
        ...
        all.objects = sessions.query(State).all()
        
        self.assertTrue(len(all_objects) > 0)
                        
    @unittest.skipIf(models.storage_t != 'db', "not testing db storage")
    def test_new(self):
        """Test that adds new data to the database"""
        state_data = {"name": "Rivers"}
        new_state = State(**state_data)
    ...
    def test_get(self):
        ...
        retrive_state = storage.get(State, state_instance.id)
        
        self.assertEqual(retrive_state.id, state_instance.id)
    ...
    def test_count(self):
        ...
        storage = models.storage
        storage.reload()
        state_data = {"name": "iceland"}
        state_instance = State(**state_data)
        storage.new(state_instance)
        
        city_data = {"name": "gardencity", "state_id": state_instance.id}
